...
class vis_odometry():
    ...
    def __init__(self):
       ...
       #variables to calculate visual odometry
        ...
        #self.detector = cv2.FastFeatureDetector_create(threshold=50, nonmaxSuppression=False)
    ...
    def start(self):
        ...
        while not rospy.is_shutdown():#main loop
            ...
            self.new_frame = self.last_frame    #Get the last Gray-frame
          ...
          ##------------------ Clear the points that are outside the image range x>=0, x<640 || y>=0, y<480 ------ This is here, because the function calcOpticalFlowPyrLK predict the points movements and they can be outside the range
           ...
           ##------------------  Check the depth in the features-points and form a numpy array --------------- 
            ...
            mask = np.logical_and(self.depth_p > 0, self.depth_p < 3500)
            ...
            mask = np.logical_and(self.depth_p_1 > 0, self.depth_p_1 < 3500)
            self.depth_p_1 = self.depth_p_1[mask]
            self.depth_p = self.depth_p[mask]
            #good_p_1 = good_p_1[mask] #aply mask to take the outsiders
            #good_p = good_p[mask] #aply the same mask on the others points, to have the same points
            ...
            # ## ----------------------------------------------------------------------------------------------------------------------------------------   
            ##------Remove Nan numbers-------------------------------------------------
            #mx = np.ma.masked_invalid(self.depth_p)
            #mx = np.ma.masked_array(self.depth_p, np.isnan(self.depth_p))
            #mx = np.logical_not(np.isnan(self.depth_p))
            #depth_p_1 = np.nan_to_num(self.depth_p_1)#where is nan -> go to 0
            #depth_p = np.nan_to_num(self.depth_p)
            #scale = np.subtract(depth_p, depth_p_1
            ## -----------------------------------------------------------------------------------------------
            ...
            if (np.count_nonzero(scale_dif==0) >= scale_dif.shape[0]*0.7): # if more than 3/4 of the numbers are  0. There is no movement
                scale = 0
            ...
            #if self.id < 2:#Inicialize for the first 2 frames
                # E, _ = cv2.findEssentialMat(good_p, good_p_1, self.focal_length, self.pp, cv2.RANSAC, self.prob, 2, None)
                # _, self.R, self.t, _ = cv2.recoverPose(E, good_p_1, good_p, self.R, self.t, self.focal_length, self.pp, None)
            ...
            # else:
                ...
                if(abs(scale) > 0.02): #and abs(t[0][0]) > abs(t[1][0]) and abs(t[0][0]) > abs(t[2][0]):
                    ...
                    if (np.any(np.logical_or(aux_dif > 0.35, aux_dif < -0.35))):# if the diference is too big, keep the old value and ignore the new [0.35 rad = 20]
                        self.R = auxR
                        self.t =auxt 
                        #print("Aqui esta um que foi arranjado!!!!")                            
                    #self.t = self.t + (abs(scale)*self.R.dot(t))           
                    #self.t = self.t + (abs(scale)*t)   
            ...
            beta = -np.arcsin(self.R[2,0])
            alpha = np.arctan2(self.R[2,1]/np.cos(beta),self.R[2,2]/np.cos(beta))
            gamma = np.arctan2(self.R[1,0]/np.cos(beta),self.R[0,0]/np.cos(beta))
            q=tf.transformations.quaternion_from_euler(beta,alpha,gamma)#gamma, beta,alpha         

            ##---------------Send the Odometry information to a node to see it in RVIZ------------##
            #self.transformT.sendTransform((0,0,0.5),q,rospy.Time.now(),"me","camera_link")         
            
            #self.transformT.sendTransform((0,0,0.5),(r,i,j,k),rospy.Time.now(),"me","camera_link") 
            #self.transformT.sendTransform(self.t,(r,i,j,k),rospy.Time.now(),"me","camera_link")            
            
            #self.transformT.sendTransform((0,0,0.5),(i,j,k,r),rospy.Time.now(),"me","camera_link")
            #self.transformT.sendTransform(self.t,(i,j,r,k),rospy.Time.now(),"me","camera_link")
            
            self.transformT.sendTransform((self.t[0],0,0),(k,i,j,r),rospy.Time.now(),"me","camera_link")#this one      
